...
int move(char *world,int target_index, int robot_index) {
    if(target_index<robot_index &&world[robot_index-21]!='#' )
    {
        return 1;
    }
    else if(target_index>robot_index && world[robot_index+21]!='#' )
    {
        return 3;
    }
...
// printf("%d %d",target_index,robot_index);
 ...
 movement = move(world,target_index,robot_index);
